# DC34 Badge BIO Uploader

Use the Python script in this repo to upload a BIO program to your DC34 badge.
...
```bash
dc34-bio --port /dev/ttyACM0 --code mycode.bin --quantum 10MHz --sao 1,2
```
...
| `--sao` | SAO I/Os to map into the BIO (expects 1, 2, 3 or 4) |
| `--quantum` | Setting for the quantum divider (accepts a number with MHz (or mhz), KHz (or khz), Hz (or hz), or bare number for Hz). Examples: 350MHz  48KHz  1000  3.5MHz.  Max: 350 MHz.
| `--clear` | Clears the current code, stops the BIO and exits (other arguments are ignored) |
...
```bash
pipx install --force git+https://github.com/bunnie/dc34-bio.git
```
...
You should see `dialout` in the list. After that, `dc34-bio` will work without `sudo`.
...
```bash
dc34-bio --port COM3 --code mycode.bin --quantum 10MHz --sao 1,2
```

> **Tip:** If you're on Windows and using WSL, the port won't be directly available inside WSL. Either run `dc34-bio` from a regular Windows terminal (PowerShell or CMD), or look into [usbipd-win](https://github.com/dorssel/usbipd-win) to forward USB devices into WSL.